...
| Entry | **GitHub Pages live** (no install) or local `--serve` | "Personal ↗" in the top-right (`?personal=1`, **requires local `--serve`**) |
...
> ⚠️ **The personal edition needs a local backend**: its radar data (profile-driven search / LLM AI explanations / questionnaire persistence / daily auto-generation) all depend on local `--serve`. The GitHub Pages live site provides the **public edition only** (data auto-generated by CI, including pipeline-generated AI explanations); opening the personal edition there shows an empty-state guide — run locally instead.

...
| | **公版**（给大家看） | **个人版**（给自己看） |
| --- | --- | --- |
| 定位 | 客观潜力发掘：同一份榜单，打开即用 | 专属推荐：只为你搜索与解读 |
...
> 一句话：公版回答「**这周哪些项目在起飞**」，个人版回答「**哪些项目适合我**」。两套排序与解读机制完全不同。
...
**长期层的生效条件**：本地运行 `python src/main.py --serve` 并打开页面，约 8 秒后自动全量同步（footer「同步数据」按钮可手动触发），此后 CI 每日用你的最新画像重算精选。若只在 GitHub Pages 上浏览且从不同步，长期层将停留在仓库内最后一次同步的画像（浏览器实时层不受影响）。
